...
if __name__ == "__main__":
    ...
    # Silakan sesuaikan metode encode dan decode Simple8bPostings berdasarkan parameter method yang Anda implementasikan
    with InvertedIndexWriter('test', encoding_method=Simple8bPostings, path='./tmp/') as index:
        index.append(1, [2, 3, 4, 8, 10])
        index.append(2, [3, 4, 5])
        index.index_file.seek(0)
        assert index.terms == [1,2], "terms salah"
        assert index.postings_dict == {1: (0, 5, len(Simple8bPostings.encode([2,3,4,8,10]))),
                                       2: (len(Simple8bPostings.encode([2,3,4,8,10])), 3,
                                           len(Simple8bPostings.encode([3,4,5])))}, "postings dictionary salah"
        assert Simple8bPostings.decode(index.index_file.read()) == [2, 3, 4, 8, 10, 13, 14, 15], "penyimpanan postings pada harddisk salah"
        
        index.index_file.seek(index.postings_dict[2][0])
        assert Simple8bPostings.decode(index.index_file.read(len(Simple8bPostings.encode([3,4,5])))) == [3,4,5], "terdapat kesalahan"

        index.index_file.seek(0)
        assert Simple8bPostings.decode(index.index_file.read(index.postings_dict[1][2])) == [2, 3, 4, 8, 10], "terdapat kesalahan"
        assert Simple8bPostings.decode(index.index_file.read(index.postings_dict[2][2])) == [3, 4, 5], "terdapat kesalahan"
